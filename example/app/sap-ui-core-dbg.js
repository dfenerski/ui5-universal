...
(function (__global) {
    ...
    function waitForNextTask() {
        if (pWaitForNextTask == null) {
            ...
            pWaitForNextTask = new Promise(function (resolve) {
                ...
                oNextTaskMessageChannel.port2.addEventListener(
                    'message',
                    function () {
                        setTimeout(function () {
    ...
    function scheduleExecution(fnCallback) {
        ...
        return function () {
            ...
            pWaitForNextTask.then(
                scheduleExecution(fnCallback).bind(undefined, arguments[0]),
            );
        };
    }
    ...
    function normalize(sResourceName, sBaseName) {
        ...
        if (p === 0) {
            if (sBaseName == null) {
                throw new Error(
                    "relative name not supported ('" + sResourceName + "'",
                );
            }
            // prefix module name with the parent package
            sResourceName =
                sBaseName.slice(0, sBaseName.lastIndexOf('/') + 1) +
                sResourceName;
        }
        ...
        if (sUrlPrefix == null) {
            ...
            if (sResourceNamePrefix) {
                if (mUrlPrefixes[sResourceNamePrefix]) {
                    ...
                    log.info(
                        `registerResourcePath ('${sResourceNamePrefix}') (registration removed)`,
                    );
                }
                return;
            }
            ...
            log.info(
                `registerResourcePath ('${sResourceNamePrefix}') (default registration restored)`,
            );
        }
    ...
    function getResourcePath(sResourceName, sSuffix) {
        ...
        assert(
            (p > 0 || sNamePrefix === '') && mUrlPrefixes[sNamePrefix],
    ...
    function guessResourceName(sURL, bLoadedResourcesOnly) {
        ...
        for (const sNamePrefix in mUrlPrefixes) {
            ...
            const sUrlPrefix = mUrlPrefixes[sNamePrefix].absoluteUrl.slice(
                0,
                -1,
            );
            ...
            if (sURL.startsWith(sUrlPrefix)) {
                ...
                if (
                    !bLoadedResourcesOnly ||
                    mModules[sResourceName]?.data != undefined
                ) {
                    return sResourceName;
                }
            }
    ...
    function getMappedName(sResourceName, sRequestingResourceName) {
        ...
        if (mMap != null) {
            ...
            if (p > 0) {
                const sMappedResourceName =
                    mMap[sPrefix] + sResourceName.slice(p);
                if (log.isLoggable()) {
                    log.debug(
                        `module ID ${sResourceName} mapped to ${sMappedResourceName}`,
                    );
                }
    ...
    function getGlobalProperty(sName) {
        ...
        if (syncCallBehavior && aNames.length > 1) {
            log.error(
                "[nosync] getGlobalProperty called to retrieve global name '" +
                    sName +
                    "'",
            );
        }
            ...
            if (this.state === READY) {
                if (this.content === NOT_YET_DETERMINED) {
                    ...
                    const oShim = mShims[this.name],
                        sExport =
                            oShim &&
                            (Array.isArray(oShim.exports)
                                ? oShim.exports[0]
                                : oShim.exports);
                    // best guess for thirdparty modules or legacy modules that don't use sap.ui.define
                    this.content = getGlobalProperty(
                        sExport || urnToUI5(this.name),
                    );
                }
                return this.content;
            }
        ...
        dependsOn(oDependantModule) {
            ...
            if (result && stack) {
                log.error(
                    ...
                    stack
                        .reverse()
                        .map((entry, idx) => `${''.padEnd(idx)} -> ${entry}`)
                        .join('\n')
                        .slice(4),
                );
            }
    ...
    let _globalDefine, _globalDefineAMD;
    ...
    function updateDefineAndInterceptAMDFlag(newDefine) {
        ...
        if (_globalDefine) {
            ...
            _globalDefine = _globalDefineAMD = undefined;
        }
            ...
            Object.defineProperty(_globalDefine, 'amd', {
                get: function () {
                    ...
                    if (sCurrentModule && mShims[sCurrentModule]?.amd) {
                        log.debug(
                            `suppressing define.amd for ${sCurrentModule}`,
                        );
                        return undefined;
                    }
    ...
    try {
        ...
        log.warning(
            "could not intercept changes to window.define, ui5loader won't be able to a change of the AMD loader",
        );
    }
    ...
    function ModuleDefinitionQueue(nested) {
        ...
        this.push = function (name, deps, factory, _export) {
                ...
                log.debug(
                    sLogPrefix +
                        ...
                        (document.currentScript
                            ? ' from ' + document.currentScript.src
                            : '') +
                        ' to define queue #' +
                        iRun,
                );
            ...
            const sModule =
                document.currentScript?.getAttribute('data-sap-ui-module');
            aQueue.push({
                name: name,
                deps: deps,
                factory: factory,
        ...
        this.process = function (oRequestedModule, sInitiator) {
            ...
            if (oRequestedModule?.execError) {
                if (bLoggable) {
                    log.debug(
                        `module execution error detected, ignoring queued define calls (${aQueueCopy.length})`,
                    );
                }
                    ...
                    if (sModuleName != null) {
                        ...
                        log.error(
                            'Modules that use an anonymous define() call must be loaded with a require() call; ' +
                                ...
                                'Now using substitute name ' +
                                oEntry.name,
                        );
                    }
                } else if (
                    oRequestedModule &&
                    oEntry.name === oRequestedModule.name
                ) {
            ...
            if (bLoggable) {
                log.debug(
                    sLogPrefix +
                        ...
                        sInitiator +
                        '] ' +
                        'processing define queue #' +
                        iCurrentRun +
                        (oRequestedModule
                            ? " for '" + oRequestedModule.name + "'"
                            : '') +
                        ` with entries [${aQueueCopy.map(
                            (entry) => `'${entry.name}'`,
                        )}]`,
                );
            }
            ...
            if (sModuleName != null && !oRequestedModule.settled) {
                ...
                if (bLoggable) {
                    log.debug(
                        sLogPrefix +
                            'no queued module definition for the requested module found, assume the module to be ready',
                    );
                }
            ...
            if (bLoggable) {
                log.debug(
                    sLogPrefix + `processing define queue #${iCurrentRun} done`,
                );
            }
        };
    }
    ...
    window.addEventListener('error', function onUncaughtError(errorEvent) {
        ...
        if (oModule && oModule.execError == null) {
            ...
            if (log.isLoggable()) {
                log.debug(
                    `unhandled exception occurred while executing ${sModuleName}: ${errorEvent.message}`,
                );
            }
    ...
    function loadScript(oModule, sAlternativeURL) {
        ...
        function onerror(e) {
            ...
            if (sAlternativeURL) {
                log.warning(
                    `retry loading JavaScript resource: ${oModule.name}`,
                );
    ...
    function preloadDependencies(sModuleName) {
        ...
        if (Array.isArray(knownDependencies)) {
            log.debug(
                `preload dependencies for ${sModuleName}: ${knownDependencies}`,
            );
    ...
    function requireModule(
        ...
        if (!oSplitName) {
            throw new Error(
                `can only require Javascript module, not ${sModuleName}`,
            );
        }
        ...
        if (sModuleName[0] == '/') {
            log.error(
                'Module names that start with a slash should not be used, as they are reserved for future use.',
            );
        }
        ...
        if (oModule.deprecation) {
            const msg =
                typeof oModule.deprecation === 'function'
                    ? oModule.deprecation()
                    : oModule.deprecation;
            log.error(
                (oRequestingModule
                    ? "(dependency of '" + oRequestingModule.name + "') "
                    : '') + msg,
            );
        }
        ...
        if (oShim?.deps && !bSkipShimDeps) {
            ...
            return requireAll(
                ...
                function (oErr) {
                    ...
                    throw oModule.failWith(
                        'Failed to resolve dependencies of {id}',
                        oErr,
                    );
                },
                bAsync,
        ...
        if (
            ...
            oModule.group &&
            oModule.group !== sModuleName &&
            !bSkipBundle
        ) {
            if (bLoggable) {
                log.debug(
                    `${sLogPrefix}require bundle '${oModule.group}' containing '${sModuleName}'`,
                );
            }
            if (bAsync) {
                return requireModule(null, oModule.group, bAsync)
                    .catch(noop)
                    .then(function () {
        ...
        if (oModule.state !== INITIAL) {
            ...
            if (oModule.state === PRELOADED) {
                ...
                measure &&
                    measure.start(
                        sModuleName,
                        'Require module ' + sModuleName + ' (preloaded)',
                        ['require'],
                    );
            ...
            if (oModule.state === READY) {
                ...
                return bAsync
                    ? Promise.resolve(wrapExport(oModule.value()))
                    : wrapExport(oModule.value());
                ...
                if (bAsync) {
                    ...
                    if (
                        oRequestingModule &&
                        oModule.dependsOn(oRequestingModule)
                    ) {
                        if (log.isLoggable()) {
                            log.debug(
                                "cycle detected between '" +
                                    oRequestingModule.name +
                                    "' and '" +
                                    sModuleName +
                                    "', returning undefined for '" +
                                    sModuleName +
                                    "'",
                            );
                        }
        ...
        measure &&
            measure.start(sModuleName, 'Require module ' + sModuleName, [
                'require',
            ]);
        ...
        const aExtensions = shouldLoadDebugVariant(sModuleName)
            ? ['-dbg', '']
            : [''];
        ...
        if (!bAsync) {
            for (
                let i = 0;
                i < aExtensions.length && oModule.state !== LOADED;
                i++
            ) {
                // create module URL for the current extension
                oModule.url = getResourcePath(
                    oSplitName.baseID,
                    aExtensions[i] + oSplitName.subType,
                );
                if (bLoggable) {
                    log.debug(
                        sLogPrefix +
                ...
                if (syncCallBehavior) {
                    const sMsg =
                        "[nosync] loading module '" + oModule.url + "'";
            ...
            try {
                ...
                if (bLoggable) {
                    if (typeof oModule.data === 'string') {
                        ...
                        log.debug(
                            sLogPrefix + "executing '" + sModuleName + "'",
                        );
                    }
                ...
                if (typeof oModule.data === 'function') {
                    ...
                    if (sScript) {
                        const oMatch =
                            /\/\/[#@] source(Mapping)?URL=(.*)$/.exec(sScript);
                        if (
                            oMatch &&
                            oMatch[1] &&
                            /^[^/]+\.js\.map$/.test(oMatch[2])
                        ) {
                            // found a sourcemap annotation with a typical UI5 generated relative URL
                            sScript =
                                sScript.slice(0, oMatch.index) +
                                oMatch[0].slice(0, -oMatch[2].length) +
                                resolveURL(oMatch[2], oModule.url);
                        }
                ...
                if (isModuleError(err)) {
                    ...
                    if (err instanceof SyntaxError && sScript) {
                        ...
                        if (fnIgnorePreload) {
                            oModule.url = URL.createObjectURL(
                                new Blob([sScript], {
                            ...
                            log.error(
                                "A syntax error occurred while evaluating '" +
                                    sModuleName +
                                    "'" +
                ...
                if (bLoggable) {
                    ...
                    log.debug(
                        sLogPrefix + "finished executing '" + sModuleName + "'",
                    );
                }
    ...
    function requireAll(
                ...
                if (oRequestingModule) {
                    switch (sDepModName) {
                        case 'require.js':
                            ...
                            aModules[i] = wrapExport(
                                createContextualRequire(sBaseName, true),
                            );
                            break;
                        case 'module.js':
        ...
        if (bAsync) {
            const oPromise = oError
                ? Promise.reject(oError)
                : Promise.all(aModules);
    ...
    function executeModuleDefinition(
        ...
        function shouldSkipExecution() {
            if (oModule.settled) {
                ...
                if (
                    ...
                    log.warning(
                        'Repeated module execution skipped after async/sync conflict for ' +
                            oModule.name,
                    );
                    return true;
                }
        ...
        function onSuccess(aModules) {
            ...
            if (bLoggable) {
                log.debug(
                    sLogPrefix +
                        "define('" +
                        sResourceName +
                        "'): dependencies resolved, calling factory " +
                        typeof vFactory,
                );
            }
            ...
            if (typeof vFactory === 'function') {
                ...
                try {
                    ...
                    if (
                        oModule._api?.exports !== undefined &&
                        oModule._api.exports !== oModule._exports
                    ) {
                    ...
                    const wrappedError = oModule.failWith(
                        "failed to execute module factory for '{id}'",
                        error,
                    );
                    if (bAsync) {
            ...
            if (bExport && syncCallBehavior !== 2) {
                if (oModule.content == null) {
                    log.error(
                        `Module '${sResourceName}' returned no content, but should export to global?`,
                    );
                } else {
                    if (bLoggable) {
                        log.debug(
                            `exporting content of '${sResourceName}': as global object`,
                        );
                    }
        ...
        requireAll(
            oModule,
            aDependencies,
            bAsync && oModule.data ? scheduleExecution(onSuccess) : onSuccess,
            function (oErr) {
                const oWrappedError = oModule.failWith(
                    'Failed to resolve dependencies of {id}',
                    oErr,
                );
                if (!bAsync) {
    ...
    function ui5Define(sModuleName, aDependencies, vFactory, bExport) {
        ...
        if (!Array.isArray(aDependencies)) {
            ...
            if (typeof vFactory === 'function' && vFactory.length > 0) {
                aDependencies = ['require', 'exports', 'module'].slice(
                    0,
                    vFactory.length,
                );
            } else {
        ...
        if (
            ...
            (bForceSyncDefines == null && bGlobalAsyncMode)
        ) {
        ...
        const oCurrentExecInfo =
            _execStack.length > 0 ? _execStack[_execStack.length - 1] : null;
        if (!sResourceName) {
            if (oCurrentExecInfo && !oCurrentExecInfo.used) {
                ...
                if (oCurrentExecInfo) {
                    sResourceName =
                        oCurrentExecInfo.name.slice(
                            0,
                            oCurrentExecInfo.name.lastIndexOf('/') + 1,
                        ) + sResourceName;
                }
                log.error(
                    'Modules that use an anonymous define() call must be loaded with a require() call; ' +
                        'they must not be executed via script tag or nested into other modules. ' +
                        'All other usages will fail in future releases or when standard AMD loaders are used ' +
                        'or when ui5loader runs in async mode. Now using substitute name ' +
                        sResourceName,
                );
            }
        } else if (
            oCurrentExecInfo?.used &&
            sResourceName !== oCurrentExecInfo.name
        ) {
            log.debug(
                `module names don't match: requested: ${sModuleName}, defined: ${oCurrentExecInfo.name}`,
            );
    ...
    function createContextualRequire(sContextName, bAMDCompliance) {
        const fnRequire = function (vDependencies, fnCallback, fnErrCallback) {
            assert(
                typeof vDependencies === 'string' ||
                    Array.isArray(vDependencies),
                'dependency param either must be a single string or an array of strings',
            );
            assert(
                fnCallback == null || typeof fnCallback === 'function',
                'callback must be a function or null/undefined',
            );
            assert(
                fnErrCallback == null || typeof fnErrCallback === 'function',
                'error callback must be a function or null/undefined',
            );

            // Probing for existing module
            if (typeof vDependencies === 'string') {
                const sModuleName = getMappedName(
                    vDependencies + '.js',
                    sContextName,
                );
                const oModule = Module.get(sModuleName);
        ...
        fnRequire.toUrl = function (sName) {
            const sMappedName = ensureTrailingSlash(
                getMappedName(sName, sContextName),
                sName,
            );
    ...
    function toUrl(sName) {
        if (sName.indexOf('/') === 0) {
            throw new Error(
                `The provided argument '${sName}' may not start with a slash`,
            );
        }
    ...
    function requireSync(sModuleName) {
        ...
        return unwrapExport(
            requireModule(null, sModuleName, /* bAsync = */ false),
        );
    }
    ...
    function predefine(sModuleName, aDependencies, vFactory, bExport) {
        ...
        Module.get(sModuleName + '.js').preload(
            '<unknown>/' + sModuleName,
            [sModuleName, aDependencies, vFactory, bExport],
            null,
        );
    }
    ...
    function dumpInternals(iThreshold) {
        ...
        const info = log.isLoggable('INFO')
            ? log.info.bind(log)
            ...
            aModuleNames.forEach((sModule, idx) => {
                ...
                if (oModule.state === state) {
                    ...
                    if (oModule.state === LOADING) {
                        ...
                        addtlInfo =
                            (oModule.error.name || 'Error') +
                            ': ' +
                            oModule.error.message;
                    }
                    info(
                        '  ' +
                            (idx + 1) +
    ...
    const mUI5ConfigHandlers = {
        ...
        async(async) {
            if (bGlobalAsyncMode && !async) {
                throw new Error(
                    'Changing the ui5loader config from async to sync is not supported. Only a change from sync to async is allowed.',
                );
            }
        ...
        noConflict(bValue) {
            log.warning(
                "Config option 'noConflict' has been deprecated, use option 'amd' instead, if still needed.",
            );
    ...
    const mAMDConfigHandlers = {
        ...
        paths(module, url) {
            registerResourcePath(
                module,
                resolveURL(url, getResourcePath('') + '/'),
            );
        },
    ...
    function handleConfigObject(oCfg, mHandlers) {
        function processConfig(key, value) {
            ...
            if (typeof handler === 'function') {
                ...
                log.warning(
                    `configuration option ${key} not supported (ignored)`,
                );
            }
        }
    ...
    const privateAPI = {
        ...
        set callbackInMicroTask(v) {
            simulateAsyncCallback = v
                ? executeInMicroTask
                : executeInSeparateTask;
        },
        get maxTaskDuration() {
        ...
        declareModule(sResourceName, fnDeprecationMessage) {
            /* void */ declareModule(
                normalize(sResourceName),
                fnDeprecationMessage,
            );
        },
        defineModuleSync,
        dump: dumpInternals,
...
(function () {
    ...
    /** BaseConfiguration */
    var ui5loader =
        globalThis.sap && globalThis.sap.ui && globalThis.sap.ui.loader;

    if (ui5loader == null) {
        throw new Error(
            'ui5loader-autoconfig.js: ui5loader is needed, but could not be found',
        );
    }
    ...
    define('sap/base/strings/_camelize', [], function () {
        var rCamelCase = /[-\.]([a-z0-9])/gi;
        var fnCamelize = function (sString) {
            var sNormalizedString = sString.replace(
                rCamelCase,
                function (sMatch, sChar) {
                    ...
                    xhr.addEventListener('load', function () {
                        try {
                            if (xhr.responseType === 'json') {
                                ...
                                globalThis['sap-ui-config'] = JSON.parse(
                                    xhr.responseText,
                                );
                            }
                        } catch (error) {
                            ui5loader._.logger.error(
                                'Parsing externalized bootstrap configuration from "' +
                                    url +
                                    '" failed! Reason: ' +
                                    error +
                                    '!',
        ...
        if (bootstrap.tag) {
            ...
            if (dataset['sapUiConfig']) {
                ...
                try {
                    ...
                    ui5loader._.logger.error(
                        'JSON.parse on the data-sap-ui-config attribute failed. Please check the config for JSON syntax violations.',
                    );
    ...
    define('sap/base/config/GlobalConfigurationProvider', [
                    ...
                    if (!sNormalizedKey) {
                        ...
                        ui5loader._.logger.error(
                            "Configuration option '" +
                                sKey +
                                "' was already set by '" +
                                mOriginalGlobalParams[sNormalizedKey] +
                                "' and will be ignored!",
                        );
                    } else if (
                        Object.hasOwn(mFrozenProperties, sNormalizedKey) &&
                        oGlobalConfig[sKey] !== vFrozenValue
                    ) {
                        oConfig[sNormalizedKey] = vFrozenValue;
                        ui5loader._.logger.error(
                            "Configuration option '" +
                                sNormalizedKey +
                                "' was frozen and cannot be changed to " +
                                oGlobalConfig[sKey] +
                                '!',
                        );
                    } else {
        ...
        function get(sKey, bFreeze) {
            ...
            if (
                !Object.hasOwn(oConfig, sKey) &&
                !Object.hasOwn(oWriteableConfig, sKey)
            ) {
                var vMatch = sKey.match(rAlias);
                var sLowerCaseAlias = vMatch
                    ? vMatch[1] +
                      vMatch[2][0] +
                      vMatch[2].slice(1).toLowerCase()
                    : undefined;
                if (sLowerCaseAlias) {
                    vValue =
                        oWriteableConfig[sLowerCaseAlias] ||
                        oConfig[sLowerCaseAlias];
                }
            }
            if (bFreeze) {
        ...
        if (globalThis.location) {
            ...
            urlParams.forEach(function (value, key) {
                ...
                if (sNormalizedKey) {
                    if (Object.hasOwn(oConfig, sNormalizedKey)) {
                        ui5loader._.logger.error(
                            "Configuration option '" +
                                key +
                                "' was already set by '" +
                                mOriginalUrlParams[sNormalizedKey] +
                                "' and will be ignored!",
                        );
                    } else {
                    ...
                    ui5loader._.logger.error(
                        "Invalid configuration option '" + key + "' in url!",
                    );
                }
            });
    ...
    define('sap/ui/base/config/MetaConfigurationProvider', [
        ...
        if (globalThis.document) {
            ...
            allMetaTags.forEach(function (tag) {
                    ...
                    if (Object.hasOwn(oConfig, sNormalizedKey)) {
                        ui5loader._.logger.error(
                            "Configuration option '" +
                                tag.name +
                                "' was already set by '" +
                                mOriginalTagNames[sNormalizedKey] +
                                "' and will be ignored!",
                        );
                    } else {
                ...
                } else if (tag.name && bSapParam) {
                    // tags without explicit name (tag.name === "") are ignored silently
                    ui5loader._.logger.error(
                        "Invalid configuration option '" +
                            tag.name +
                            "' in meta tag!",
    ...
    define('sap/base/config/_Configuration', [
                ...
                switch (vType) {
                    case TypeEnum.Boolean:
                        if (typeof vValue === 'string') {
                            return (
                                vValue.toLowerCase() === 'true' ||
                                vValue.toLowerCase() === 'x'
                            );
                        } else {
                            vValue = !!vValue;
                        }
        ...
        function checkEnum(oEnum, sValue, sPropertyName) {
            ...
            throw new TypeError(
                'Unsupported Enumeration value for ' +
                    sPropertyName +
                    ', valid values are: ' +
                    aValidValues.join(', '),
            );
        ...
        function get(mOptions) {
            if (
                typeof mOptions.name !== 'string' ||
                !rValidKey.test(mOptions.name)
            ) {
                throw new TypeError(
                    "Invalid configuration key '" + mOptions.name + "'!",
                );
            }
            if (mOptions.name === 'sapUiOnInit') {
                return 'module:sap/ui/core/ComponentSupport';
            }
            if (mOptions.name === 'sapUiResourceRoots') {
                return {
            ...
            if (!(sCacheKey in mCache)) {
                ...
                var bIgnoreExternal =
                    bGlobalIgnoreExternal || !mOptions.external;
                var sName = mOptions.name;
                var vMatch = sName.match(rXXAlias);
                var vDefaultValue = mOptions.hasOwnProperty('defaultValue')
                    ? mOptions.defaultValue
                    : mInternalDefaultValues[mOptions.type];

                const aAllProvider = [
                    ...aProvider,
                    ...(mOptions.provider ? [mOptions.provider] : []),
                ];

                for (var i = aAllProvider.length - 1; i >= 0; i--) {
                    if (!aAllProvider[i].external || !bIgnoreExternal) {
                        const vProviderValue = convertToType(
                            aAllProvider[i].get(sName, mOptions.freeze),
                            mOptions.type,
                            mOptions.name,
                        );
                        if (vProviderValue !== undefined) {
                            if (mOptions.type === TypeEnum.MergedObject) {
                                vValue = Object.assign(
                                    {},
                                    vProviderValue,
                                    vValue,
                                );
            ...
            if (
                typeof mOptions.type !== 'function' &&
                (mOptions.type === TypeEnum.StringArray ||
                    mOptions.type === TypeEnum.Object ||
                    mOptions.type === TypeEnum.MergedObject)
            ) {
    ...
    function _setupConfiguration() {
        var BaseConfiguration = sap.ui.require(
            'sap/base/config/_Configuration',
        );
        //register config provider
        BaseConfiguration.registerProvider(
            sap.ui.require('sap/ui/core/config/BootstrapConfigurationProvider'),
        );
        BaseConfiguration.registerProvider(
            sap.ui.require('sap/ui/base/config/MetaConfigurationProvider'),
        );
        BaseConfiguration.registerProvider(
            sap.ui.require('sap/ui/base/config/URLConfigurationProvider'),
        );
    }
    ...
    /** autoconfig */
    var sBaseUrl, bNojQuery, aScripts, rBootScripts, i, sBootstrapUrl;
    ...
    if (
        !findBaseUrl(
            document.querySelector('SCRIPT[src][id=sap-ui-bootstrap]'),
            /^((?:[^?#]*\/)?resources\/)/,
        )
    ) {
    ...
    if (sBaseUrl == null) {
        throw new Error(
            'ui5loader-autoconfig.js: could not determine base URL. No known script tag and no configuration found!',
        );
    }
    ...
    (function () {
        var sRebootUrl;
        try {
            // Necessary for FF when Cookies are disabled
            sRebootUrl = window.localStorage.getItem('sap-ui-reboot-URL');
        } catch (e) {
            /* no warning, as this will happen on every startup, depending on browser settings */
        }
        ...
        if (sRebootUrl) {
            var sDebugRebootPath =
                ensureSlash(sBaseUrl) + 'sap/ui/core/support/debugReboot.js';
    ...
    (function () {
        ...
        try {
            vDebugInfo =
                vDebugInfo || window.localStorage.getItem('sap-ui-debug');
        } catch (e) {
        ...
        window['sap-ui-optimized'] =
            window['sap-ui-optimized'] ||
            (/\.getAttribute/.test(findBaseUrl) &&
                !/oScript/.test(findBaseUrl));
        ...
        if (window['sap-ui-optimized'] && vDebugInfo) {
            ...
            if (vDebugInfo === true && !window['sap-ui-debug-no-reboot']) {
                ...
                if (sBootstrapUrl != null) {
                    sDebugUrl = sBootstrapUrl.replace(
                ...
                var oRestart = new Error(
                    'This is not a real error. Aborting UI5 bootstrap and restarting from: ' +
                        sDebugUrl,
                );
                oRestart.name = 'Restart';
                throw oRestart;
        ...
        if (typeof vDebugInfo === 'string') {
            var sPattern =
                    '^(?:' +
                    vDebugInfo.split(/,/).map(makeRegExp).join('|') +
                    ')',
            ...
            ui5loader._.logger.debug(
                "Modules that should be excluded from preload: '" +
                    sPattern +
                    "'",
            );
            ...
            ui5loader._.logger.debug(
                'All modules should be excluded from preload',
            );
        }
    ...
    let sNoSync = BaseConfig.get({
        // call must be made to ensure freezing
        name: 'sapUiXxNoSync',
        type: BaseConfig.Type.String,
        external: true,
        freeze: true,
     ...
     */
    (() => {
        const GlobalConfigurationProvider = sap.ui.require(
            'sap/base/config/GlobalConfigurationProvider',
        );
    ...
    ui5loader.config({
        ...
        shim: {
            ...
            'sap/viz/ui5/container/libs/sap-viz-controls-vizcontainer': {
                deps: [
                    'sap/viz/libs/sap-viz',
                    'sap/viz/ui5/container/libs/common/libs/rgbcolor/rgbcolor_static',
                ],
            },
